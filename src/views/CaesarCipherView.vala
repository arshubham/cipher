...
public class CaesarCipherView : Gtk.Grid  {
    ...
    public CaesarCipherView () {
        ...
        enchiper_button.clicked.connect (() => {
            ...
            cipherText = "";
            ciphertext_textview.buffer.text = caesar.encrypt (plainText, spinbutton.get_value_as_int ());
        });

        dechiper_button.clicked.connect (() => {
            cipherText = ciphertext_textview.buffer.text;
            plainText = "";
            plaintext_textview.buffer.text = caesar.decrypt (cipherText, spinbutton.get_value_as_int ());
        });
    }
